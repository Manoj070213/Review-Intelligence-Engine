...
The full source CSV (~265 MB) is not included in this repo due to size. Download it from [Kaggle](https://www.kaggle.com/datasets/datafiniti/consumer-reviews-of-amazon-products), place it at `amazon_reviews_real.csv`, and the pipeline will regenerate everything. The processed sample (`raw_reviews.csv`) is included so the pipeline runs immediately from step 2 without the download.
...
python 01_load_real_data.py     # load source reviews, map to pipeline schema
python 02_clean_and_extract.py  # clean messy data + LLM extraction
python 03_sql_analysis.py       # load SQLite, run aggregation queries
python 04_validate.py           # measure extraction accuracy vs labeled set
# then open dashboard.html in a browser
```

## How the extraction works

The core of the project is the prompt in `build_prompt()` (in `02_clean_and_extract.py`). It constrains the model to return a **fixed JSON schema** with a **closed list of theme labels**, rather than free-text analysis.
...
python 02_clean_and_extract.py
```
...
- `01_load_real_data.py` — load source reviews into the pipeline schema
- `01_generate_data.py` — synthetic data generator (runs with no external dataset)
- `02_clean_and_extract.py` — data cleaning + LLM extraction (the core)
- `03_sql_analysis.py` — SQL aggregation and business queries
- `04_validate.py` — accuracy measurement vs. a labeled set
- `dashboard.html` — interactive dashboard
- `dashboard_data.json` — aggregated tables (ready for any BI tool)
